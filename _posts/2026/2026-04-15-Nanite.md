...
构造 leaf cluster 的核心逻辑是：根据划分结果 `Partitioner.Ranges` 中每个 `Range` 记录的 index 范围 `[Begin, End)`，根据 `SortedIndexes[index]` 转换为原始 mesh 中的三角形 index，把三角形每个 vertex 的原始顶点索引映射到 cluster 局部顶点索引，并复制顶点的 Position/Normal/TangentX/TangentYSign/Color/UV/BoneIndices/BoneWeights 属性。每个 cluster 中：

- Cluster 局部顶点数组 `Verts`；Cluster 局部顶点索引 `Indexes`；Cluster 中每个三角形材质索引 `MaterialIndexes`
- Cluster 的外部邻接信息 `ExternalEdges`，每条有向边对应 `ExternalEdges` 中的一个 `int8`：值为 0 则说明此条边在 cluster 内部；值大于 0 则表示此条边在 cluster 边界上。而 `NumExternalEdges` 统计的则是**处在 cluster 边界的边的数量**
...
到这里，Nanite 完成了 leaf cluster 的构造。而下一步要做的，就是从 leaf cluster 开始，一级一级往上构建整个 **Cluster DAG（Directed Acyclic Graph，有向无环图）****
...
首先构建 leaf cluster，每个 cluster 包含 128 个三角形，这一步目前已经完成。

然后从 leaf cluster（**也就是 LOD 0**）开始，反复将当前 LOD 层的 cluster 按**拓扑邻接/空间局部性邻接**划分成多个 groups，每个 group 内把所有 children cluster 合并成一个临时大 mesh，然后将此 mesh **简化到约一半三角形数量**，最后将简化后的大 mesh 重新切分成新的 parent cluster；将这些 parent cluster 作为下一层 LOD 输入继续处理，直到只剩一个 root cluster，这个 root cluster 单独构成一个 root group。

继续引用 UE 分享的一个精简版关于**合并 group 内 children cluster -> 简化 -> 切分 parent cluster**的示意图：
