...
下一步要做的，就是并行处理所有 group，首先将每个 group 中的 children cluster 合并成一个 merged cluster，然后对 merged cluster 进行简化，最后将简化后的 merged cluster 重新切分成若干 parent cluster

**当前 LOD 层所有 group 经过合并、简化再切分后生成出来的所有 parent cluster 构成了 LOD+1 层的 cluster**
