...
在原始的 mesh 中，顶点索引数据 `Indexes` 中记录了所有三角形的 vertex index，每 3 个 vertex index 是一个三角形，Nanite 按 vertex index 取每条有向边，`EdgeIndex` 实际就是 `Indexes` 中的每个 vertex index，后续在 mesh 简化的源代码中，`CornerIndex` 也是每个 vertex index。
...
最后，也就是根据 `Partitioner.Ranges` 创建所有的 leaf cluster：
    ...
    const FAdjacency& Adjacency )
{
    ...
    // 使用 Indexes 中的分区范围生成 leaf cluster GUID
    ...
    {
        // Begin/End 是按 Morton 编码排序后的 SortedIndexes 里的区间起始/终点, 而不是原始三角形 index, 真正的原始三角形 index 是 SortedIndexes[i]
        ...
        for( uint32 k = 0; k < 3; k++ )
        {
            ...
            Adjacency.ForAll( EdgeIndex,
                [ &AdjCount, Begin, End, &SortedTo ]( int32 EdgeIndex, int32 AdjIndex )
                {
                    ...
                    // 确认这个邻接三角形在不在当前 cluster 的 [Begin, End) 区间中, 如果不在则表明此邻接三角形属于另一个 cluster, 也就是说明这条有向边与其它 cluster 邻接, 是它所在 cluster 的边界
...
构造 leaf cluster 的核心逻辑是：根据划分结果 `Partitioner.Ranges` 中每个 `Range` 记录的 index 范围 `[Begin, End)`，根据 `SortedIndexes[index]` 转换为原始 mesh 中的三角形 index，把三角形每个 vertex 的原始顶点索引映射到 cluster 局部顶点索引，并复制顶点的 Position、Normal、Color、UV 等属性。

每个 cluster 中：

- cluster 局部顶点数组 `Verts`；Cluster 局部顶点索引 `Indexes`；Cluster 中每个三角形材质索引 `MaterialIndexes`
- cluster 的外部邻接信息 `ExternalEdges`，每条有向边对应 `ExternalEdges` 中的一个 `int8`：值为 0 则说明此条边在 cluster 内部；值大于 0 则表示此条边在 cluster 边界上；而 `NumExternalEdges` 统计的则是**处在 cluster 边界的边的总数量**
- cluster 的 AABB `Bounds`；Cluster 的包围球 `SphereBounds`； leaf cluster 初始 LOD bounds `LODBounds = SphereBounds`；Cluster 内所有三角形面积和 `SurfaceArea`；Cluster 内最大三角形边长 `EdgeLength`

Nanite 通过**Ritter 包围球算法（Ritter’s Bounding Sphere Algorithm）**计算 cluster 的包围球 `SphereBounds`，在这里额外贴一下 Ritter 包围球算法的代码：
...
第二步（上图中），把这个 group 内的 4 个 children cluster 合并成一个大 mesh 并对其进行简化；

最后（上图右），将简化后的大 mesh 重新切分成橙色和粉红色这 2 个 parent cluster，这 2 个 parent cluster 是 LOD1 层；
...
我们知道，Nanite 在构建 leaf cluster 时，会先构建原始 mesh 中三角形之间的**拓扑邻接关系**和**空间局部性邻接关系**，然后构建三角形图，图中的每个节点代表每个三角形，再根据拓扑邻接/空间局部性邻接设置图节点之间的权重，最终使用图划分库 METIS 将三角形划分到不同的 cluster。

同样的，对 cluster 划分 group 也是相似的逻辑，只不过图节点从三角形变成了 cluster，划分的核心代码在 `FClusterDAG::GroupTriangleClusters()` 方法中：
...
<!-- #### 1.2.2. 合并 Group 内的 Children Cluster -->
