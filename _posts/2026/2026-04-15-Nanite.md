...
构造叶子 Cluster 的核心逻辑是：根据划分结果 `Partitioner.Ranges` 中每个 `Range` 记录的 index 范围 `[Begin, End)`，根据 `SortedIndexes[index]` 转换为原始 mesh 中的三角形 index，把三角形每个 vertex 的原始顶点索引映射到 Cluster 局部顶点索引，并复制顶点的 Position/Normal/TangentX/TangentYSign/Color/UV/BoneIndices/BoneWeights 属性。每个 Cluster 中：
...
- Cluster 的 AABB `Bounds`；Cluster 的包围球 `SphereBounds`；叶子 Cluster 初始 LOD bounds `LODBounds = SphereBounds`；Cluster 内所有三角形面积和 `SurfaceArea`；Cluster 内最大三角形边长 `EdgeLength`
...
到这里，Nanite 完成了叶子 Cluster 的构造。而下一步要做的，就是从叶子 Cluster 开始，一级一级往上，构建 **Cluster DAG（Directed Acyclic Graph，有向无环图）****
