...
3. 在 Nanite 编码前，还会对 cluster 做编码约束调整：在每个 `MaterialRange` 内对三角形进行重排，并按新的访问顺序重排顶点；当某个旧顶点超出 32-index 窗口时，会复制该顶点以生成新的局部顶点索引；如果约束后 cluster 顶点数超过 256，还会将 cluster 按三角形范围拆分并重新约束。这样可以保证每个 cluster 满足 32-index 窗口和 256 顶点上限，为后续 index 编码和 GPU transcoding 做准备。
    ...
    void BuildTables( const FCluster& Cluster )
    {
        ...
        for( uint32 i = 0; i < NumTriangles; i++ )
        {
            ...
            // 把三角形 i 写进其 3 个顶点的 VertexToTriangleMasks
            // 其中: uint32 VertexToTriangleMasks[NANITE_MAX_CLUSTER_TRIANGLES * 3 ][ MAX_CLUSTER_TRIANGLES_IN_DWORDS ]
            // 一共 128 个三角形, 这里用 4 个 uint32 的每个 bit 来做标记, 每个三角形占 1 bit
            // i >> 5 计算出三角形 i 属于第几个 uint32; 1 << ( i & 31 ) 计算出此三角形在所属 uint32 中的具体 bit 位; 最后将此 bit 位置为 1
            ...
            Node0.Corner = (uint16)SetCorner( i, 0 );   // Corner 低 2 位存储 local corner index (0..2), 高 14 位存储的是三角形 index
            Node0.NextNode = EdgeNodeHeads[ i1 * NANITE_MAX_CLUSTER_INDICES + i2 ];
            // corner 0 对面的边是 (i1, i2), 所以把 corner 0 挂到有向边 i1 -> i2 的链表上
    ...
    可以看到 `BuildTables` 会先构建 3 张表：

    - `VertexToTriangleMasks`：记录**每个旧顶点关联了哪些三角形**，通过 `VertexToTriangleMasks[ vertex index ][ triangle i >> 5 ]` 可以知道三角形 `i` 是否使用了顶点 `Verts[index]`；
    - `OppositeCorner`：记录每个三角形 corner 对应**跨边**相邻三角形的 corner，如果没有就是 `INVALID_CORNER`，后续 strip 延伸依赖此数据找左/右相邻三角形；
    - `TrianglePriorities`：后面选 strip 起点时，如果 score 相同，用它决定谁优先；
    ...
    {
        // 当前这个旧顶点是否已经进入新的顶点序列; NewIndex=INVALID_INDEX 表示还没有，或者已经因为窗口超过 32-index 而被强制失效
        uint16 NewIndex = Context.OldToNewVertex[ OldVertex ];

        // 默认分数是 0, 也就是说如果这个顶点不能复用, 则它对候选三角形没有加分
        int32 CacheScore = 0;
        if( NewIndex != INVALID_INDEX )
        {
            // 计算这个顶点距离当前最新顶点的距离
            uint32 CachePosition = ( Context.NumVertices - 1 ) - NewIndex;
            // 判断这个旧顶点是否在 Nanite 允许的 32-index 窗口内
            if( CachePosition < NANITE_CONSTRAINED_CLUSTER_CACHE_SIZE )
            {
                // 如果顶点还在 32-index 窗口里, 就从权重表取分数, 根据一下 5 个维度查表:
                // bStart: 候选三角形是否作为新 strip 的起点
                // bHasOpposite: 该三角形对边方法是否还有可用邻居
                // bHasLeft: 左侧是否还能继续延伸
                // bHasRight: 右侧是否还能继续延伸
                // CachePosition: 顶点在受限窗口里的相对位置
    ...
    auto NewScoreTriangle = [ &Cluster, &NewScoreVertex ] ( const FContext& Context, uint32 TriangleIndex, bool bStart, bool bHasOpposite, bool bHasLeft, bool bHasRight )
    {
        ...
        // 通过 NewScoreVertex 分别对 3 个顶点打分, 求出当前三角形的分数
    ...
    通过 `NewScoreVertex` 判断某个旧顶点是否能被当前 strip 编码复用，如果能，则根据此顶点在 32-index 窗口的缓存距离和拓扑信息计算这个顶点作为复用候选的一个贡献分，这个分数同时考虑了**顶点的复用价值**和**拓扑延伸潜力**。

    而 `NewScoreTriangle` 则是通过分别对三角形的 3 个顶点计算贡献从而得到三角形的贡献分
    ...
    {
        // LocalCorner = 2, 则返回 LocalCorner = 0 的 Corner
        if( ( Corner & 3 ) == 2 )
            Corner &= ~3;
        else
            Corner++;   // LocalCorner = 0, 则返回 LocalCorner = 1 的 Corner; LocalCorner = 1, 则返回 LocalCorner = 2 的 Corner;
        return Corner;
    }

    static FORCEINLINE uint32 PrevCorner( uint32 Corner )
    {
        // LocalCorner = 0, 则返回 LocalCorner = 2 的 Corner
        if( ( Corner & 3 ) == 0 )
            Corner |= 2;
        else
            Corner--;   // LocalCorner = 1, 则返回 LocalCorner = 0 的 Corner; LocalCorner = 2, 则返回 LocalCorner = 1 的 Corner;
        return Corner;
    }

    static FORCEINLINE uint32 CornerToIndex( uint32 Corner )
    {
        // 三角形 index * 3 + LocalCorner index
    ...
    {
        // 首先明确输入参数的含义:
        // Context: 整个 stripify 过程的可变状态
        // TriangleCorner: 被选中三角形 corner, 这个 corner 编码了三角形 index 和 LocalCorner, **Corner = (Triangle << 2) | LocalCorner**
        // bStart: 这个三角形是否是新 strip 的起点
        // bRight: 如果不是起点, 那么它是从右边接上来的还是左边接上来的

        // 按 i0 -> i1, i1 -> i2, i2 -> i0 的顺序取下一个顶点, 具体取决于 OldIndex2
        const uint32 OldIndex0 = Cluster.Indexes[ CornerToIndex( NextCorner( TriangleCorner ) ) ];
        // 按 i0 <- i1, i1 <- i2, i2 <- i0 的顺序取上一个顶点, 具体取决于 OldIndex2
        const uint32 OldIndex1 = Cluster.Indexes[ CornerToIndex( PrevCorner( TriangleCorner ) ) ];
        // TriangleCorner 中编码的 LocalCorner
        ...
        if( bStart )
        {
            ...
            // 起点三角形里有多少个顶点是引用顶点, 复用顶点需要写 5-bit delta
        ...
        {
            // 给 OldIndex0 分配新的顶点 index
            NewIndex0 = uint16(Context.NumVertices++);
            // 并记录新顶点 index 对应旧顶点 OldIndex0
