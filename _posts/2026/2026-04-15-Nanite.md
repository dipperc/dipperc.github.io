...
- [1. 离线构建：Nanite Pages](#1-离线构建nanite-pages)
  ...
  - [1.2. 构建 Cluster DAG](#12-构建-cluster-dag)
    ...
    - [1.2.2. 合并 Group 内的 Children Cluster](#122-合并-group-内的-children-cluster)
...
构造 leaf cluster 的核心逻辑是：根据划分结果 `Partitioner.Ranges` 中每个 `Range` 记录的 index 范围 `[Begin, End)`，根据 `SortedIndexes[index]` 转换为原始 mesh 中的三角形 index，把三角形每个 vertex 的原始顶点索引映射到 cluster 局部顶点索引，并复制顶点的 Position/Normal/TangentX/TangentYSign/Color/UV/BoneIndices/BoneWeights 属性，最后将构造的 leaf cluster 添加到 `ClusterDAG::Clusters` 数组中

每个 cluster 中包含：

1. **基础 mesh 信息**：cluster 局部顶点数组 `Verts` 和顶点索引 `Indexes`；cluster 中所有三角形的材质索引 `MaterialIndexes`
2. **与其它 cluster 之间的拓扑邻接信息**：`ExternalEdges` 中的每个 `int8` 元素记录 cluster 中一条有向三角形边的**外部邻接边数量**：值为 0 表示该边没有连接到 cluster 外部的邻接三角形；值大于 0 表示该边与 cluster 外部的三角形邻接，因此是 cluster 边界边。而 `NumExternalEdges` 记录了 cluster 中边界边的总数量
3. **包围盒信息**：cluster 的 AABB `Bounds`；包围球 `SphereBounds`；LOD 包围范围 `LODBounds`，在 leaf cluster 中其初始为 `LODBounds = SphereBounds`
4. **其它 mesh 信息**：cluster 内所有三角形的面积和 `SurfaceArea`；cluster 内所有三角形中最长的边的边长 `EdgeLength`
...
对 cluster 划分 group 也是相似的逻辑：先构建当前 LOD 层所有 cluster 之间拓扑邻接关系和空间局部性邻接关系，然后根据图划分将所有 cluster 划分到不同的 group，核心逻辑如下：
    ...
    uint32 NumExternalEdges )
{
    ...
    {
        uint32  ClusterRefIndex;    // cluster 索引
        int32   EdgeIndex;          // cluster 内有向三角形边索引
    };
    TArray< FExternalEdge > ExternalEdges;
    FHashTable              ExternalEdgeHash;
    TAtomic< uint32 >       ExternalEdgeOffset(0);

    // cluster 的拓扑邻接关系数组, 每个 cluster 对应其中的一个 TMap< uint32, uint32 > 元素
    ...
    ExternalEdges.AddUninitialized( NumExternalEdges ); // 这里的 NumExternalEdges 代表的是所有 cluster 的边界边的数量总和
        ...
        [&]( uint32 ClusterRefIndex )
        {
            ...
            for( int32 EdgeIndex = 0; EdgeIndex < Cluster.ExternalEdges.Num(); EdgeIndex++ )
            {
                ...
                if( Cluster.ExternalEdges[ EdgeIndex ] )
                {
                    ...
                    // hash table 中以有向三角形边 hash 为 key, value 是 ExternalEdges 中的索引, 根据此索引可以快速获取有向三角形边的 FExternalEdge 信息
...
void FClusterDAG::GroupTriangleClusters( TArrayView< const FClusterRef > LevelClusters, uint32 NumExternalEdges )
{
    ...
    // 当前层的 cluster 数量少于 32, 则把当前层的所有 cluster 放进一个 group
