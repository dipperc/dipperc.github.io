...
将每套 UV 的朝向符号编码进 `MaterialIndexes` 是因为：底层 `FMeshSimplifier` 在判断一条边是否属于属性连续的内部边时，会要求两侧三角形的 `MaterialIndexes` 完全相同。因此，UV 镜像两侧虽然可能使用同一个材质，但因为 UV 朝向符号不同，仍会被当作属性断层从而对其添加**边界约束（Edge Constraint）**，这样就可以避免把 UV 镜像处**塌陷（Collapse）**简化从而造成贴图反转的问题。
...
```text
Normal.xyz      -> 1.0f                                 // 法线: 非常重要
Tangent.xyz     -> 0.0625f                              // 切线: 较重要
Tangent.w       -> 0.5f                                 // 副切线方向: 重要
Color.xyzw      -> 0.0625f                              // 顶点颜色: 较重要
UVs             -> 1.0f / (128.0f * TriangleUVSize)     // UV: 按 UV 尺度归一化
BoneIndex       -> 0.0f                                 // BoneIndex: 权重为 0
BoneWeight      -> 0.0f                                 // BoneWeight: 权重为 0
```

计算 UV 权重时，会根据每套 UV 的总面积对 UV 面积做归一化，这样可以避免出现：UV 很密集时（面积小），误差巨大，不敢对其进行简化；UV 很稀疏时（面积大），误差很小，简化之后把 UV 破坏的很厉害。

而 Skinning 相关的属性（BoneIndex/BoneWeight）权重为 0 是因为：在做**边塌陷（Edge Collapse）**简化时，新生成的顶点的属性通常都是通过线性插值得到的，但是 Skinning 属性与其它属性不同，它是离散数据，是不能做插值的，例如对 BoneIndex 做插值可能会得到一个非法的 BoneIndex（`BoneIndex = lerp(12, 3, 0.5)`）；而对 BoneWeight 做插值会破坏其归一化（插值之后所有 BoneWeight 的和不再是 1）。
...
对于 group 内部边，简化时 Nanite 允许自由塌陷；而对于 group 的边界边，Nanite 会将其两个端点锁定，在做边塌陷简化时，如果这条边的一个端点被锁定、另一个端点未被锁定，那么新位置必须落在被锁定的端点；而如果两个端点都被锁定，那么评估合并时会施加非常大的惩罚（LockPenalty）。

通过这种对 group 的外边界施加**强约束式保护（正常情况下不会破坏外边界，但并非逻辑上的绝对禁止）**的方式可以尽量保证相邻 group 的 parent clusters 之间不会出现**裂缝（Crack）**。

//TODO: Simplifier.Simplify 主要做了什么

<!-- ##### 1.2.2.3. Split -->

<!-- ---
//TODO: 关于 `LODError` 和 `ParentLODError` 的分析, 最后补充说明

每个 cluster 自己都有一个 `LODError`，这个误差表示的是：当前这个 cluster 作为某一级 LOD 表示时，相对更细一级已经积累了多少几何误差。

而 `ReduceGroup` 核心在做的是：把当前 LOD 层级 group 中的所有 children cluster 合并、简化、再切分成下一层级的 parent cluster，parent cluster 表示的是”更粗一级“的 children cluster

所以 parent cluster 的 `LODError` 不能只看简化产生了多少误差，还必须覆盖 children 的 `LODError`，这样可以保证**误差由子层到父层是单调递增的**

并且因为一个 group 的 parent cluster 是由同一批 children cluster 合并、简化、再切分出来的，运行时不能只把其中一个 parent 切回 children，而另一个 parent 还停留在父层，这样可能会导致 group 内部边界裂开，所以 Nanite 以 group 作为 LOD 切换单位：

- parent clusters 都指向同一个 `GeneratingGroupIndex`
- 如果运行时决定展开这个 parent group，就展开 `Groups[GeneratingGroupIndex].Children`
- 同一 group 生成的 parent clusters 共享 `LODError` 和 `LODBounds` 从而保证它们一起切换
--- -->

<!-- #### 1.2.3. RootCluster and RootGroup -->
...
<!-- ---
`FCluster::Simplify` 的核心作用可以概括成一句话：

> 它把一个由若干更细 cluster 合并得到的临时网格，在保留边界连接关系、材质/UV 断层和重要顶点属性的条件下，用 QEM 化简成更粗的几何，并返回这次化简引入的几何误差，供 Nanite 构建 LOD DAG 使用。

实现位于 [Cluster.cpp:549](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Cluster.cpp:549)，底层真正执行边折叠的是 [MeshSimplify.cpp:943](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/MeshSimplifier/Private/MeshSimplify.cpp:943)。

**它处于 Nanite 哪个阶段**

Nanite 离线构建时，会将一组 child clusters 合并成一个临时 `Merged` cluster，然后把它化简为更粗的一层 parent clusters：

```text
多个细粒度 child clusters
        |
        v
FCluster(DAG, Group.Children)    合并为临时大 cluster
        |
        v
Merged.Simplify(...)             降低三角数并计算误差
        |
        v
SplitCluster(...)                重新切成 <= 128 tris 的 parent clusters
        |
        v
更高一级 DAG 节点
```

对应调用在 [ClusterDAG.cpp:894](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/ClusterDAG.cpp:894)：
...
见 [ClusterDAG.cpp:1070](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/ClusterDAG.cpp:1070)。

它还被用于 fallback/coarse mesh 生成，见 [NaniteBuilder.cpp:410](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/NaniteBuilder.cpp:410)。
...
`TargetNumTris`：希望最终降到的三角形数量。

`TargetError`：希望化简至少达到的误差目标。普通 DAG 父级生成时默认为 `0`，因此主要由三角数目标控制；fallback/trim 会传入相对误差换算出的世界空间误差。

`LimitNumTris`：不可继续低于的三角数量下限，主要用于 fallback mesh。

`RayTracingFallbackBuildSettings`：光追 fallback 专用的体积/遮挡调整配置。

需要注意：底层 `FMeshSimplifier` 的停止条件是：
...
见 [MeshSimplify.cpp:1008](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/MeshSimplifier/Private/MeshSimplify.cpp:1008)。

所以 `Target*` 是需要共同满足的目标；`Limit*` 则是硬停止界限。

---

**第一步：无需化简时直接退出**

[Cluster.cpp:551](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Cluster.cpp:551)：
...
如果当前 cluster 已经不超过目标三角数，并且没有额外误差要求，就不用处理。

---

**第二步：把 UV 镜像边伪装成材质断层**

[Cluster.cpp:556](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Cluster.cpp:556) 会遍历每个三角形的每套 UV：
...
MaterialIndexes[ TriIndex ] |=
    ( SignedArea >= 0.0f ? 1 : 0 ) << ( UVIndex + 24 );
```

这里做了两件事：

1. 统计每套 UV 的总面积，稍后用于计算 UV 属性权重。
2. 把 UV 三角形的朝向符号编码进 `MaterialIndexes` 的高位。

为什么要修改材质编号？

底层 simplifier 在判断一条边是否属于连续内部边时，要求两侧三角形的 `MaterialIndexes` 完全相同，见 [MeshSimplify.cpp:117](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/MeshSimplifier/Private/MeshSimplify.cpp:117)。

因此，UV 镜像两侧虽然可能使用同一个真实材质，但因为 UV 朝向符号不同，会被当成属性断层处理。这样化简就不会轻易跨过 mirrored UV seam 折叠，避免法线贴图或纹理在镜像边附近产生明显错误。
...
见 [Cluster.cpp:736](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Cluster.cpp:736)。

---

**第三步：对位置进行尺度归一化**

[Cluster.cpp:583](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Cluster.cpp:583)：

```cpp
float TriangleSize = FMath::Sqrt( SurfaceArea / (float)NumTris );
```

它用平均三角形面积估计平均线性尺寸，然后构造一个缩放因子，使平均三角形尺寸接近 `0.25`：
...
这里特意只调整 IEEE float 的 exponent，因此缩放尽量保持数值精度，不引入普通乘法比例带来的额外尾数误差。

目的不是改变模型尺寸，而是让 QEM 化简器处理的坐标落在比较稳定的数值范围内：
...
化简完成后位置会缩放回来，返回误差也会恢复到原始世界空间单位。

---

**第四步：为各类顶点属性配置误差权重**

`FMeshSimplifier` 不只考虑位置误差，也考虑 normal、tangent、color、UV 等属性误差。

顶点布局定义在 [Cluster.h:46](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Cluster.h:46)；权重设置位于 [Cluster.cpp:601](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Cluster.cpp:601)。
...
| Tangent handedness  |    `0.5` | 镜像切线方向不能随意错           |
...
- UV 密度很高时，一个小的 UV 位移可能对应明显纹理偏差，权重相应调整。
- 不同 UV scale 的模型不会因为展开比例不同而出现完全不同的化简行为。

如果 `DAG.Settings.bLerpUVs == false`，UV 权重会设为 `0`。编辑器设置对此的说明是：某些模型把 UV 通道当作离散索引或自定义数据，不能插值，见 [EngineTypes.h:3051](/Users/czw/Desktop/UnrealEngine/Engine/Source/Runtime/Engine/Classes/Engine/EngineTypes.h:3051)。

骨骼影响始终设为 `0`，因为 bone index 和 bone weight 不能像普通连续属性一样随意线性混合；simplifier 会采用邻近原顶点的数据。
...
`ExternalEdges` 表示这个 cluster 与当前合并区域之外仍然相接的边。函数会找到这些边的两个端点，并锁住它们：
...
见 [Cluster.cpp:658](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Cluster.cpp:658)。

底层 `LockPosition` 会给所有使用该空间位置的 corner 加上 `LockedVertMask`，见 [MeshSimplify.cpp:59](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/MeshSimplifier/Private/MeshSimplify.cpp:59)。

边折叠时：

- 一个端点锁定、另一个未锁定：新位置必须落在锁定端点。
- 两个端点都锁定：合并会被施加极高惩罚。

见 [MeshSimplify.cpp:399](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/MeshSimplifier/Private/MeshSimplify.cpp:399)。

这保证了：

```text
一个 group 内部可以大胆简化
group 与外部邻居共享的边界不能随便移动
```

否则相邻 parent clusters 或 LOD 切换区域可能出现裂缝、不连续轮廓或无法正确重建 DAG 邻接关系。

---

**第六步：配置并运行 QEM 化简器**

[Cluster.cpp:677](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Cluster.cpp:677)：

```cpp
Simplifier.SetAttributeWeights( AttributeWeights );
Simplifier.SetCorrectAttributes( CorrectAttributesFunctions[...] );
Simplifier.SetEdgeWeight( 2.0f );
Simplifier.SetMaxEdgeLengthFactor( DAG.Settings.MaxEdgeLengthFactor );
```

`SetCorrectAttributes` 会在生成新顶点属性后：

- 重新归一化 normal；
- 将 tangent 正交化并归一化；
- 将 tangent sign 重新限制为 `-1` 或 `1`；
- clamp vertex color。

代码在 [Cluster.cpp:18](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Cluster.cpp:18)。

`MaxEdgeLengthFactor` 用于抑制过长的新边，特别适合将来还会发生 WPO、Spline 或动画变形的网格。设置字段的官方注释位于 [EngineTypes.h:3120](/Users/czw/Desktop/UnrealEngine/Engine/Source/Runtime/Engine/Classes/Engine/EngineTypes.h:3120)。
...
- `TargetNumVerts = Verts.Num()`，等价于不依赖顶点数作为主要目标；
- `TargetNumTris` 是主要几何预算；
- 误差以平方形式传给 QEM；
- `LimitNumTris` 是不可突破的三角形下限；
- 误差上限为 `MAX_flt`，表示该层没有硬性的最大误差截断。

---

**底层具体怎么化简**

`FMeshSimplifier` 是带属性的 Quadric Error Metric 边折叠器。

它首先为每个三角形建立包含属性影响的 quadric：

```cpp
FQuadricAttr(
    Position0, Position1, Position2,
    Attributes0, Attributes1, Attributes2,
    AttributeWeights );
```

见 [MeshSimplify.cpp:100](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/MeshSimplifier/Private/MeshSimplify.cpp:100)。

然后为边界、材质断层和 UV 镜像断层建立额外 edge quadric：

```cpp
EdgeQuadrics[ EdgeIndex ] =
    FEdgeQuadric( Position0, Position1, Weight );
```

见 [MeshSimplify.cpp:117](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/MeshSimplifier/Private/MeshSimplify.cpp:117)。

之后，每条可合并边都会计算一个代价，放入最小堆：
...
它还会拒绝明显导致三角翻面的移动，并用 `MaxEdgeLengthFactor` 提高产生长边的折叠误差，见 [MeshSimplify.cpp:383](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/MeshSimplifier/Private/MeshSimplify.cpp:383) 和 [MeshSimplify.cpp:552](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/MeshSimplifier/Private/MeshSimplify.cpp:552)。

---

**第七步：植被、体积与光追 fallback 的特殊处理**

完成普通化简后，函数还可能修改剩余几何的形状。

如果正在生成 ray tracing fallback，并配置了 `FoliageOverOcclusionBias`：

```cpp
if( bHasVoxelTriangles )
{
    Simplifier.ShrinkVoxelTriangles(...);
}
else
{
...
见 [Cluster.cpp:689](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Cluster.cpp:689)。

这是因为简化后的 foliage 往往会比原模型更加密实，光追时造成过度遮挡。这里通过向三角形中心适当收缩几何，降低 fallback 的过遮挡问题。

否则，如果设置为 `PreserveArea`：

```cpp
Simplifier.PreserveSurfaceArea();
```

见 [Cluster.cpp:700](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Cluster.cpp:700)。

`PreserveArea` 是旧式 foliage 保形策略：尽量补偿化简造成的表面积损失。`Voxelize` 则是在更远层级用体素化方式尽量保留 perceived volume，枚举说明见 [EngineTypes.h:3025](/Users/czw/Desktop/UnrealEngine/Engine/Source/Runtime/Engine/Classes/Engine/EngineTypes.h:3025)。

---

**第八步：压紧结果并恢复 Nanite 需要的数据**

化简器内部会留下被删除的顶点和三角形槽位，因此之后调用：

```cpp
Simplifier.Compact();
```

它重新连续排列活跃顶点、重写 index buffer，并压缩材质数组，见 [MeshSimplify.cpp:1386](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/MeshSimplifier/Private/MeshSimplify.cpp:1386)。

随后 `FCluster::Simplify` 更新：
...
其中 `ExternalEdges` 不能简单沿用旧索引，因为三角形拓扑已经改变。因此它利用之前保存的锁定有向边，按最终顶点位置重新识别边界：

```cpp
auto Edge = MakeTuple( NewPosition0, NewPosition1 );
if( LockedEdges.Find( Edge ) )
{
    ExternalEdges[ EdgeIndex ] = *AdjCount;
}
```

见 [Cluster.cpp:714](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Cluster.cpp:714)。
...
`Simplify` 不只是在压缩三角形数量，它还为 Nanite 的运行时 LOD 决策生成误差依据。

在 DAG 构建时，一组 child clusters 生成若干 parent clusters。父层表示的是更粗的近似几何，因此 parent 必须知道：

```text
用我替代 children，最多可能偏离多少
```

这个量就是 `LODError` 的来源之一。

构建时：

```cpp
Group.ParentLODError =
    Max(已有 child 的误差, 当前 SimplifyError);
```

这保证误差随 DAG 向上保持单调，不会发生“更粗层反而宣称误差更小”的情况。

运行时 Nanite 遍历 hierarchy 时，就可以根据：
...
判断当前视角应该继续细分，还是直接渲染这个 parent cluster。

---

**常规 DAG 化简与 fallback 化简的区别**

常规 DAG 父级生成：

```cpp
Simplify( DAG, TargetNumTris );
```

特点：

- `TargetError = 0`；
- 目标主要是把合并后的 child 减到 parent 容量预算内；
- 结果之后重新拆成若干 `<= 128` 三角形的父 cluster；
- 返回误差写入 DAG LOD 数据。

Fallback/coarse mesh：
...
Simplify(
    ...
    TargetNumTris,
    TargetError,
    Min(TargetNumTris, 256u),
    RayTracingFallbackBuildSettings );
```

特点：

- 同时考虑最终 triangle budget 与用户配置的相对误差；
- 保留一定三角下限；
- 可能额外针对光追植被收缩几何；
- 输出给传统渲染、碰撞或 ray tracing fallback 使用，而不是继续形成 Nanite DAG 层级。

---

**最值得记住的设计点**

`FCluster::Simplify` 不是“对一个 cluster 做一次普通减面”，而是 Nanite 离线 LOD 体系里的约束化简适配层：

1. 它把多个 child cluster 的几何变成 parent 层粗表示。
2. 它用 QEM 同时衡量位置和可插值顶点属性误差。
3. 它保护外边界，避免 cluster/group 之间产生裂缝。
4. 它把 UV 镜像边临时转为不可随意跨越的属性断层。
5. 它处理 foliage、voxel、ray tracing fallback 等特殊保形需求。
6. 它产生 `SimplifyError`，最终成为运行时选 LOD 的关键依据。

因此，Nanite 的 parent cluster 并不是简单抽稀后的三角片，而是“在可流式加载、可局部替换、可稳定做运行时 LOD 决策前提下”的受约束近似表示。
--- -->
