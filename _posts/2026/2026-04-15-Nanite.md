...
- [1. 离线构建：Nanite Pages](#1-离线构建nanite-pages)
  ...
  - [1.4. `Encode` Cluster DAG](#14-encode-cluster-dag)
...
### 1.4. `Encode` Cluster DAG

到这里 Nanite 已经构建好 cluster DAG，而现在要做的就是把 cluster DAG 转换成运行时可用的 `Nanite::FResources` 数据结构。

首先会检查 cluster 数据//TODO
...
[@superpowers](plugin://superpowers@openai-curated) [Nanite_SIGGRAPH_2021_Notes.md](Engine/AIStuffs/Nanite_SIGGRAPH_2021_Notes.md) [ClusterDAG.cpp](Engine/Source/Developer/NaniteBuilder/Private/ClusterDAG.cpp) [Cluster.cpp](Engine/Source/Developer/NaniteBuilder/Private/Cluster.cpp) [NaniteEncode.cpp](Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp) 详细分析一下 Encode 方法中的逻辑，整个逻辑分为哪几块，每一块都是在干什么

`Encode(...)` 可以理解成 Nanite Builder 的“最终打包流水线”：输入是已经构建好的 `FClusterDAG`，输出是运行时可直接使用的 `FResources`，包括 root page、streaming page、hierarchy、fixup、量化参数、bulk data 等。主入口在 [NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:1155)。
...
[NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:1163)

先调用 `EncodeAssemblyData(ClusterDAG, Resources)`。

这块处理 Nanite Assembly/实例化相关数据：

- 把 `ClusterDAG.AssemblyInstanceData` 里的 transform 转成 GPU 侧更好读的 `4x3 transposed` 格式，写入 `Resources.AssemblyTransforms`。
- 如果 assembly instance 有骨骼附着信息，会构建 `Resources.AssemblyBoneAttachmentData`。
- 单骨骼影响会直接塞进 header，避免 shader 多一次间接读取。
- 多骨骼影响则把 offset 写进 header，实际 influence 数据追加到 attachment data 后面。

所以这一块不是几何压缩，而是给后续 hierarchy / runtime instance 解码准备实例变换和骨骼附着表。

**2. Cluster 数据清洗和基础约束**

对应：

[NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:1170)

这一段依次做：

- `Cluster.Verts.Sanitize()`：清理 vertex 数据，比如处理非法 float、NaN、Inf 之类的问题。
- `RemoveDegenerateTriangles(...)`：删除退化三角形，也就是三角形三个 index 里有重复点的情况。
- `BuildMaterialRanges(...)`：按 material 把 cluster 的三角形分段，生成 material range。
- `ConstrainClusters(...)`：对 cluster 做编码约束调整，比如顶点数、三角形数、material range 等必须满足 Nanite page/cluster 格式限制。
- `VerifyClusterConstraints(...)`：check 配置下验证约束结果。
- `BuildVertReuseBatches(...)`：按 material range 构建 vertex reuse batch 信息，供后续编码/解码更高效地复用顶点。

这一块的目标是：把 DAG 里的 cluster 修整成“可被 Nanite 固定格式编码”的状态。

**3. 位置、法线、切线、骨骼权重精度选择和量化**

对应：

[NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:1205)

位置量化由 `CalculateQuantizedPositionsUniformGrid(...)` 完成，核心在 [NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:191)。

它做几件事：

- 从 `Settings.PositionPrecision` 取用户设置；如果是 auto，就根据 mip0 cluster 的平均 bounds 尺寸估一个 precision。
- clamp 到 Nanite 允许的范围。
- 检查每个 cluster 的 bounds 是否能装进 `NANITE_MAX_POSITION_QUANTIZATION_BITS`，如果装不下就降低 precision。
- 对每个 vertex position 做 `round(Position * QuantizationScale)`。
- 记录每个 cluster 的：
  - `QuantizedPositions`
  - `QuantizedPosStart`
  - `QuantizedPosBits`
  - `QuantizedPosPrecision`
- 同时把 float position 替换成量化后再反解的值，让后续 bounds / 属性跟最终编码一致。

随后主函数选择：

[NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:1211)

- `Resources.NormalPrecision`：默认 8 bits。
- `Resources.TangentPrecision`：如果有 tangent 默认 7 bits，否则 0。
- `BoneWeightPrecision`：默认 8 bits。
- 如果有 skinning，调用 `QuantizeBoneWeights(...)`。

这一块是“全局精度决策 + cluster 内位置量化”。

**4. 计算每个 Cluster 的编码预算和编码元数据**

对应：

[NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:1245)

调用 `CalculateEncodingInfos(...)`，实际逻辑在 [NaniteEncodeGeometryData.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncodeGeometryData.cpp:344)。

`FEncodingInfo` 是后面分页和写 page 的核心依据。它会计算：

- index 每个索引要多少 bit：`BitsPerIndex = ceil(log2(NumClusterVerts))`。
- GPU page 每个 section 的大小：
  - `Cluster`
  - `MaterialTable`
  - `VertReuseBatchInfo`
  - `DecodeInfo`
  ...
  - `BoneInfluence`
  - `BrickData`
- attribute 编码需要多少 bit：
  - normal：`2 * NormalPrecision`，使用 octahedron 编码。
  - tangent：`1 + TangentPrecision`。
  - color：如果所有颜色相同走 constant，否则记录 min 和每通道 bit 数。
  - UV：先用 Nanite 自定义 UV float 编码，再记录每个 UV channel 的 min 和 range bit 数。
  - skinning：计算 bone influence 数量、bone index bits、weight bits。
- 最后根据 `QuantizedPosBits` 算 position bitstream 大小。

简单说，这一步还没有真正把数据写成 bitstream，而是先算“每个 cluster 要占多少空间、每种属性怎么编码”。

**5. 分页：把 ClusterGroup/Cluster 装进 Root Page 和 Streaming Page**

对应：

[NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:1250)

调用 `AssignClustersToPages(...)`，主要逻辑在 [NaniteEncodePageAssignment.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncodePageAssignment.cpp:119)。

流程是：

- 根据 `Settings.TargetMinimumResidencyInKB` 算最多 root pages。
- 先对 group 排序：mip level 优先，再按 Morton 空间顺序，让 page 顺序更局部，也避免循环依赖。
- 遍历 group 的 children，把 cluster 一个个尝试塞进当前 page。
- `TryAddClusterToPage(...)` 会用 `EncodingInfo.GpuSizes` 累加 page 尺寸，并检查：
  - root page 是否超过 `NANITE_ROOT_PAGE_GPU_SIZE`
  - streaming page 是否超过 `NANITE_STREAMING_PAGE_GPU_SIZE`
  - cluster 数是否超过 page 最大 cluster 数
- 如果塞不下，就开新 page。
- 同时生成 `FClusterGroupPart`：一个 group 可能跨多个 page，所以 page 内属于同一 group 的连续 cluster 会成为一个 part。
- 对有 instance 引用的 group，还会把 instanced cluster 所在 page 合并进 group 的 `PageRangeKey`，必要时生成 multi-range。
...
- `Cluster.PageIndex`
- `Group.PageRangeKey`
- `Resources.PageRangeLookup`
- `Resources.NumRootPages`

也就是 Nanite streaming 的基本 page 拓扑。

**6. Mesh Bounds、Hierarchy、Page 依赖和 Fixup**

对应：

[NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:1257)

先 `CalculateMeshBounds(...)`，计算最终 `Resources.MeshBounds`。如果 group 是 assembly part，还会把实例 transform 后的 sphere bounds 纳入总 sphere radius。

然后：

[NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:1262)

`BuildHierarchies(...)` 生成运行时裁剪和 LOD 选择用的 hierarchy nodes。它会把 `GroupParts` 作为叶子，构建 BVH-like hierarchy，并写入 `Resources.HierarchyNodes`。

接着：

[NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:1267)

`CalculatePageDependenciesAndFixups(...)` 计算 page 之间的依赖关系和 streaming fixup。核心含义是：

- 如果某个 group 的 child page 被 stream in，需要知道它依赖哪些 parent page。
- 最后一个 page 通常持有 group fixup。
- fixup 会告诉 runtime：
  - group page 载入后如何安装到 hierarchy。
  - 哪些 parent cluster 要从 leaf 变成 non-leaf。
  - 哪些 page 需要 reconsider。
- 生成 `Resources.PageStreamingStates` 和 `Resources.PageDependencies`。

再之后 `CalculateFinalPageHierarchyDepth(...)` 给 page 填最终 hierarchy depth，写进 streaming state。

**7. 写 Page：真正把 Cluster 编成 Nanite page 二进制布局**

这是最大的一块，对应：

[NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:1277)

调用 `WritePages(...)`，主体从 [NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:543) 开始。

它内部又可以分几小步。

第一步，构建 page 级 vertex map：

[NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:558)

`BuildVertexMaps(...)` 为每个 page 建一个 `Vertex -> (LocalClusterIndex, VertexIndex)` 的 map。后面相对编码时，可以引用 parent page 或当前 page 已经出现过的 vertex，减少重复写顶点。

第二步，决定哪些 page 用 relative encoding：

[NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:450)

`MarkRelativeEncodingPages(...)` 根据 page dependency 图决定 page 是否允许引用依赖 page 的 vertex 数据。规则大概是：

- root page 永远不用 relative encoding。
- 只依赖 root page 的 streaming page 可以用 relative encoding。
- 如果依赖链太长，超过 `MAX_DEPENDENCY_CHAIN_FOR_RELATIVE_ENCODING`，会强制改回 direct encoding，避免 runtime decode 出现太长的串行依赖链。

第三步，并行处理每个 page：

[NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:568)

对 page 内每个 cluster：

- `PackCluster(...)` 把 cluster header 打包成 `FPackedCluster`，包含顶点数、三角形数、bounds、LOD error、position quantization 参数、attribute 参数、UV offset、bone influence 数等。
- 设置 index / position / attribute / decode info / bone / brick / extended data 在 page GPU layout 里的 offset。
- 写 material table。
- 写 vertex reuse batch info。
- 如果是 voxel/brick cluster，写 brick data。
- 调用 `EncodeGeometryData(...)` 真正编码几何数据。

`EncodeGeometryData(...)` 是 page 内最关键的“几何 bitstream 编码”，在 [NaniteEncodeGeometryData.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncodeGeometryData.cpp:478)。

它做：

- 先判断能不能用 vertex refs。普通三角形 cluster 可以，voxel 暂时跳过。
- 对每个 vertex，先查依赖 page 的 `PageVertexMaps`，再查当前 page 已编码过的 `UniqueVertices`。
- 找到重复 vertex 就不再写完整属性，而是写：
  - vertex ref bitmask
  - page-cluster pair map
  - vertex ref delta
- 找不到就加入 `UniqueVertices`，后续完整写 position/attribute。
- 写 index 数据。当前配置通常走 `NANITE_USE_STRIP_INDICES`，写 `StripBitmask` 和 `StripIndexData`。
- 写 position：量化 position 做 delta，`ShortestWrap + ZigZag`，拆进 low/mid/high byte streams。
- 写 normal：octahedron encode 后做 delta，写 low/mid/high streams。
- 写 tangent：基于 normal 的切线角度编码，delta 写入。
- 写 color：如果 variable color，基于 `ColorMin` 写 delta。
- 写 UV：基于 `UVInfo.Min` 写 delta。
- 写 skinning：bone index / weight 用 bit writer 写入 `Streams.BoneInfluence`。

这里的 low/mid/high byte stream 很重要：它不是简单按 bit 连续打包，而是把不同显著字节分流，方便后续压缩和 GPU transcoding。

第四步，page 内 fixup：

[NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:715)

`PerformPageInternalFixup(...)` 会直接修正 `PackedClusters` 的 flags，比如：

- root group 标记。
- streaming leaf / root leaf 是否仍然成立。
- full leaf 标记。

也就是说，某些 leaf 状态能在当前 page 内确定，就直接写进 cluster header；跨 page 的状态交给 fixup chunk。

第五步，按磁盘/page layout 写二进制：

从 [NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:719) 到 [NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:982)。

写入顺序大致是：

1. `FPageDiskHeader` 占位。
2. 每个 cluster 的 `FClusterDiskHeader` 占位。
3. GPU page header。
4. `FPackedCluster`，并且是 SOA layout：先写所有 cluster 的第 0 个 float4，再写所有 cluster 的第 1 个 float4，以此类推，方便 GPU 读取。
5. cluster bone influence / voxel bone influence，也是 SOA layout。
6. material table。
7. vertex reuse batch info。
8. bone data。
9. brick data。
10. extended data。
11. decode info：UV header、bone influence header。
12. index data 和 strip bitmask。
13. page-cluster map。
14. vertex ref bitmask。
15. vertex refs，高低字节拆开写，利于压缩。
16. low/mid/high byte streams。
17. 回填 page disk header 和 cluster disk headers。

**8. 写入最终 Resources：RootData 和 StreamablePages**

对应：

[NaniteEncode.cpp](/Users/czw/Desktop/UnrealEngine/Engine/Source/Developer/NaniteBuilder/Private/Encode/NaniteEncode.cpp:987)

`WritePages(...)` 最后把每个 page 的数据放到最终资源里：

- root page 写入 `Resources.RootData`。
- streaming page 先写入临时 `StreamableBulkData`，最后拷贝到 `Resources.StreamablePages`。
- 每个 page 前面先写 fixup chunk，再写 page data。
- 更新 `PageStreamingState.BulkOffset`、`BulkSize`、`PageSize`。
- 统计 root / streaming 的 GPU size、disk size、fixup size。
- 如果 `OutTotalGPUSize` 不为空，返回总 GPU page size。

所以最后 `Resources` 里已经包含运行时 Nanite 所需的：
...
- material/voxel mask
- assembly transform/bone attachment data

一句话总结：`Encode(...)` 不是只做“压缩几何”，而是把 `FClusterDAG` 转换成 Nanite runtime streaming 格式。前半段修整和量化 cluster，中段根据编码尺寸分页并构建 hierarchy/dependency，后半段把 cluster header、索引、顶点属性、vertex refs、fixup 和 page bulk data 按 GPU/磁盘布局写进 `FResources`。
