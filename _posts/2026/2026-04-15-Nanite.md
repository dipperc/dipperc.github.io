...
3. 在 Nanite 编码前，还会对 cluster 做编码约束调整：在每个 `MaterialRange` 内对三角形进行重排，并按新的访问顺序重排顶点；当某个旧顶点超出 32 顶点窗口约束时，会复制该顶点以生成新的局部顶点索引；如果约束后 cluster 顶点数超过 256，还会将 cluster 按三角形范围拆分并重新约束。这样可以保证每个 cluster 满足 32 顶点窗口约束和 256 顶点上限，为后续 index 编码和 GPU transcoding 做准备。
    ...
    Nanite 是按材质段（`MaterialRange`）分段进行 stripify 的，这样可以保持材质分组不被打乱，避免 strip 跨材质。
    ...
    {
        // 确保 Cluster.MaterialRanges 中三角形是按不同材质段连续排列
        check( RangeStart == MaterialRange.RangeStart );
        // 当前材质段三角形数量
        uint32 RangeLength = MaterialRange.RangeLength;

        // Enable triangles from current range
        // 将当前材质段中的三角形标记为候选, 并将其在 Context.TrianglesEnabled 中对应的 bits 标记为 1
        // Context.TrianglesEnabled 是 4 个 uint32, 每个 uint32 标记 32 个三角形, 一共 128 个三角形
        for( uint32 i = 0; i < MAX_CLUSTER_TRIANGLES_IN_DWORDS; i++ )
        {
            // 当前材质段起点在当前 DWORD 内的 bit 偏移
            int32 RangeStartRelativeToDword = (int32)RangeStart - (int32)i * 32;
            // 当前 DWORD 内材质段起点对应的 bit
            int32 BitStart = FMath::Max( RangeStartRelativeToDword, 0 );
            // 当前 DWORD 内材质段终点对应的 bit
            ...
            // 最后按位异或将当前材质段三角形对应的 bits 置为 1, 举个例子: StartMask = bits 0..9, EndMask = bits 0..14, 那么 StartMask ^ EndMask = bits 10..14
        ...
        while( true )
        {
            ...
            {
                // 遍历所有 TrianglesEnabled 中记录的候选的三角形
                    ...
                    while( CandidateMask )
                    {
                        ...
                        for( uint32 Corner = 0; Corner < 3; Corner++ )
                        {
...

                            {
                                ...
                                // 按 strip 编码需要的局部顺序获取三角形的 3 个旧顶点 Corner
                                ...
                                // 模拟 32 顶点窗口约束: 如果旧顶点已经在新顶点序列出现过, 但输出当前三角形后, 它距离最新顶点太远导致 5-bit delta 引用不了, 则把它当作新顶点输出
                                while(true)
                                {
                            ...
                            // 查询这个 strip 起点周围的拓扑情况
                            uint32 Opposite = OppositeCorner[ CornerToIndex( TriangleCorner ) ];                    // 当前 Corner 对面的边邻居
                            uint32 LeftCorner = OppositeCorner[ CornerToIndex( NextCorner( TriangleCorner ) ) ];    // 后续从左侧延伸 strip 的邻居
                            uint32 RightCorner = OppositeCorner[ CornerToIndex( PrevCorner( TriangleCorner ) ) ];   // 后续从右侧延伸 strip 的邻居

                            // 只考虑当前材质段内尚未输出的邻居三角形, 不考虑跨材质段的邻居
...

            {
                ...
                // 输出前的最大新顶点索引, 用来计算 ref delta
                uint32 BaseVertex = Context.NumVertices - 1;
                
                // 输出这个三角形
                uint32 NumNewVertices = VisitTriangle( Context, StartCorner, true, false );

                // 为 strip 起点三角形写 ref 顶点的 5-bit delta:
                // 如果 NumNewVertices = 3, 则表示 3 个顶点都是新顶点, 不写任何 5-bit delta
                // 如果 NumNewVertices = 2, 则表示第 1 个顶点是 ref 顶点, 写 1 个 5-bit delta
                // 如果 NumNewVertices = 1, 则表示前 2 个顶点是 ref 顶点, 写 2 个 5-bit delta
                // 如果 NumNewVertices = 0, 则表示 3 个顶点都是 ref 顶点, 写 3 个 5-bit delta
                if( NumNewVertices < 3 )
                {
            ...
            while( true )
            {
                ...
                // 每个 strip 编码的元数据是 **按 32 个三角形一个 DWORD 分块** 的, 避免出现 DWORD 开头三角形不会出现依赖上一个 DWORD 的非起点三角形的情况
...

                {
                    ...
                    // 延伸三角形必须沿当前 strip 的边接上, 所以前 2 个顶点必须已经存在于新顶点序列中
                    check( NewIndex0 != INVALID_INDEX );
                    check( NewIndex1 != INVALID_INDEX );
        ...
        // 更新 RangeStart, 开始 stripify 下一个材质段
