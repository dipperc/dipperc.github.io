...
3. 在 Nanite 编码前，还会对 cluster 做编码约束调整，通过在每个 `MaterialRanges` 内对三角形重排、顶点重排、必要时复制顶点和拆分 cluster 等方式，保证每个 cluster 满足 32-index 窗口和 256 顶点上限，为后面 index 编码做准备。注：以下仅对 `ConstrainAndStripifyCluster()` 方法做分析理解：
