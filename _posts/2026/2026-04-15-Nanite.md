...
- 每次二分时，会把当前子图的目标分区大小设置为 `(MinPartitionSize + MaxPartitionSize) / 2` ；在叶子 Cluster 构建中就是 `(124 + 128) / 2 = 126`
- 对当前子图 `Graph->Num` ，计算目标最终分区数： `Max(2, DivideAndRoundNearest(Graph->Num, TargetPartitionSize))`
- 根据这个目标分区数给二分两侧设置权重（例如目标 5 个分区时，权重是 `2/5` 和 `3/5`）
- 调用 `METIS_PartGraphRecursive` 把当前图二分，根据二分结果把 `Indexes` **原地重排**成两段
- 二分后如果两段大小都不超过 `MaxPartitionSize` ，则直接把两段作为最终分区加入结果；否则为两段构建子图，每个子图的 `Offset` 中记录这 2 段在 `Indexes` 中的起始偏移，并递归二分仍然超过 `MaxPartitionSize` 的子图
- 如果当前子图 `Graph->Num <= MaxPartitionSize` ，此时把整个子图视作一个分区加入结果中

最终结果存储在 `Partitioner.Ranges` 中，它记录的是**最终分区在 `Partitioner.Indexes` 这个重排数组里的连续区间**，每个元素 `FRange` 表示一个分区在 `Indexes` 中的索引范围， `FRange::Begin` 是分区起点， `FRange::End` 是分区终点， `FRange::Num` 是这个分区里面三角形的数量，真正的原始三角形 index 需要通过 `Partitioner.Indexes[Begin..End]` 获取

#### 1.1.5. 根据分区结果创建 Leaf Clusters

最后，根据分区 `Partitioner.Ranges` 的结果，创建叶子 Clusters ：
...
下面，详细说说创建 Cluster ，每个 Cluster 中到底包含哪些数据？并且这些数据都有什么作用呢？

//TODO

到这里， Nanite 完成了叶子 Cluster 的创建。而下面要做的，就是根据叶子 Cluster 构建 Cluster 有向无环图（Cluster Directed Acyclic Graph, Cluster DAG）。
