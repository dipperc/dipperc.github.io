...
构造 leaf cluster 的核心逻辑是：根据划分结果 `Partitioner.Ranges` 中每个 `Range` 记录的 index 范围 `[Begin, End)`，根据 `SortedIndexes[index]` 转换为原始 mesh 中的三角形 index，把三角形每个 vertex 的原始顶点索引映射到 cluster 局部顶点索引，并复制顶点的 Position/Normal/TangentX/TangentYSign/Color/UV/BoneIndices/BoneWeights 属性
