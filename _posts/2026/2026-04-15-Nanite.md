...
- [因此，Nanite 的 parent cluster 并不是简单抽稀后的三角片，而是“在可流式加载、可局部替换、可稳定做运行时 LOD 决策前提下”的受约束近似表示。](#因此nanite-的-parent-cluster-并不是简单抽稀后的三角片而是在可流式加载可局部替换可稳定做运行时-lod-决策前提下的受约束近似表示)
...
Nanite 在**保留边界连接关系、材质/UV 断层和重要顶点属性的条件下**，用 **QEM（Quadric Error Metrics）**对 merged cluster 做 mesh 简化，**简化引入的几何误差被当作后续切分出的 parent cluster 的 `LODError` 使用**，简化的核心逻辑在 `FCluster::Simplify()` 和 `FMeshSimplifier::Simplify()` 方法中。

Nanite 首先会统计每套 UV 的总面积，并且把 UV 镜像标记编码进 `MaterialIndexes` 的高位：
...
if( Verts.Format.NumTexCoords > 0 )
{
    for( uint32 TriIndex = 0; TriIndex < NumTris; TriIndex++ )
    {
        ...
        for( uint32 UVIndex = 0; UVIndex < Verts.Format.NumTexCoords; UVIndex++ )
        {
            ...
            // MaterialIndexes[ TriIndex ] 的高 8 位中的每 1 位记录了每个 UV 通道的 UV 镜像标记, 低 24 位记录的还是材质 ID
...
统计每套 UV 的总面积是为了后续做 UV 属性权重的归一化，避免出现 //TODO
...
---
[Nanite_Codex_Context_Handoff_2026-04-21.md](Engine/AIStuffs/Nanite_Codex_Context_Handoff_2026-04-21.md) [Cluster.cpp](Engine/Source/Developer/NaniteBuilder/Private/Cluster.cpp) 详细说说Nanite中的FCluster::Simplify这个函数是在干什么
